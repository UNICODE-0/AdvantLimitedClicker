...
namespace Leopotam.EcsLite {
    ...
    public interface IEcsApplicationPauseSystem : IEcsSystem {
        void Pause (IEcsSystems systems, bool status);
    }
    ...
    public class EcsSystems : IEcsSystems {
        ...
        {
            foreach (var system in _allSystems) {
                if (system is IEcsApplicationPauseSystem pauseSystem) {
                    pauseSystem.Pause(this, status);
#if DEBUG && !LEOECSLITE_NO_SANITIZE_CHECKS
                    var worldName = CheckForLeakedEntities (this);
                    if (worldName != null) { throw new System.Exception ($"Empty entity detected in world \"{worldName}\" after {pauseSystem.GetType ().Name}.Pause()."); }
#endif
                }
            }
        }
